    ...
    <section className="bg-gradient-to-r from-[#DFF5F7] via-[#DFF5F7] to-[#F0F7F0] min-h-screen flex items-center justify-center">
      <div className="max-w-6xl mx-auto px-4 w-full">
        <div className="mb-12 mt-10">
          ...
          <div className="bg-[#2EA148] rounded-full h-3">
            <div className="bg-[#2EA148] h-3 rounded-full" style={{ width: "100%" }} />
          </div>
        </div>
        ...
        <div className="max-w-md mx-auto mb-12 space-y-3">
          {learningGoals.map((goal) => (
            <button
              ...
              className={`w-full p-3 rounded-xl border border-[#BDDBD9] text-center ${
        ...
        <div className="flex justify-center gap-4">
          ...
          <button onClick={handleFinish} disabled={!selectedGoal} className={`px-8 py-4 font-semibold rounded-lg ${selectedGoal ? "bg-[#2EA148] text-white" : "bg-gray-400 text-white cursor-not-allowed"}`}>Start Learning</button>
        </div>
      </div>
    </section>
  );
